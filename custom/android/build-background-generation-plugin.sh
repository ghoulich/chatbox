...
output_dir=${1:-"$script_dir/background-dex-out"}
stub_classes=$(mktemp -d /tmp/chatbox-background-stubs-XXXXXX)
plugin_classes=$(mktemp -d /tmp/chatbox-background-classes-XXXXXX)
plugin_jar=$(mktemp /tmp/chatbox-background-plugin-XXXXXX.jar)
mkdir -p "$output_dir"
...
javac -source 8 -target 8 -cp "$android_jar:$stub_classes" -d "$plugin_classes" \
  "$script_dir/BackgroundGenerationPlugin.java" \
  "$script_dir/BackgroundGenerationService.java"
...
echo "Background generation DEX written to $output_dir"
