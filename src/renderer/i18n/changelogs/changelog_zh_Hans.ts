const changelog = `
## v1.23.2 - 2026.09.10
1. 远程 MCP 服务支持 OAuth 授权，可在配置窗口连接服务、查看和刷新工具列表
2. 对话压缩支持自定义提示词和临时补充要求，并优化压缩范围与触发判断
3. 消息用量新增缓存命中 Token 展示，Claude 支持设置提示词缓存时长
4. 支持上传更多纯文本文件，包括 Typst、Patch 和 Diff 文件
5. 归档对话后支持一键撤销，改善异常会话的启动恢复体验
6. 修复部分回复完成后仍显示生成中的问题
7. 改善生成图片的加载体验，加载失败时可重新加载
8. 支持 GPT-6 及后续模型的思考强度设置，改善 Kimi、OpenCode 和模型列表兼容性
9. 修复鸿蒙备份导出问题，并优化窗口焦点、小屏显示及多语言文案

## v1.23.1 - 2026.09.02
1. 在较长对话里，若删除历史消息 / 切换模型操作会清空已缓存的上下文，会先请你确认
2. Copilot 提示词上限提升至 4 万字符
3. 修复 Android 上会话设置菜单被系统导航栏挡住的问题
4. 修复快捷键、会话设置和无效「最大输出 Token」导致的异常

## v1.23.0 - 2026.09.01
1. 新增持久记忆：新对话也能记住你留下的关键事实，支持全局记忆或为单个搭档单独配置
2. 工作模式新增 Soul 设置，用于定义 AI 的性格和行为底线
3. 生成过程中支持排队发送新消息，也可立即插入
4. 工作目录和权限沿用上次配置，在输入框上方即可快速修改
5. 「完全访问」模式默认不再每 25 步暂停确认
6. 支持视觉的模型可自动打开本地图片或刚下载的文件进行识别
7. 优化长任务上下文保持：工具结果不再过早丢弃，上下文压缩时优先保留最近对话
8. 归档对话支持一键清空；工具调用结果中的文本内容可直接预览
9. 新增腾讯混元、小米 MiMo、LongCat、智谱 GLM Coding Plan、OpenCode Zen / OpenCode Go，并支持自托管 SearXNG 搜索引擎

## v1.22.3 - 2026.08.09
1. 修复长对话中重新生成回复或切换回复分支时出现卡顿的性能问题
2. 提升备份导出兼容性，修复部分设备无法正常导出备份的问题

## v1.22.2 - 2026.08.06
1. 优化经典消息布局与整体界面，支持自定义主题色和配色预设
2. 将多个候选回复整理为可折叠分支，方便对比并保持对话上下文清晰
3. 优化工作模式的长任务执行、审批交互和命令兼容性
4. 新增 Claude Opus 5 支持，并提升 DeepSeek 思考模式兼容性
5. 修复文件解析、自动滚动、Mermaid 图表显示等问题，并提升整体稳定性

## v1.22.1 - 2026.07.28
1. 新增工作模式：AI 可运行代码、调用工具、处理电脑中的文件，还可查找历史对话和生成图片，自主完成复杂任务
2. 新增技能（Skills）：可安装和管理可复用的 AI 能力，内置技能库持续更新
3. 新增完整数据备份，可一并导出和恢复对话、图片及附件
4. 重新设计模型选择器，支持搜索及按模型设置思考深度
5. 新增对话小地图，方便浏览长对话
6. 全新新手引导，提供开箱即用的示例
7. 支持归档对话，并在侧边栏显示对话创建时间
8. 移动端支持 PDF 文件
9. 优化回复的流式显示、自动滚动和思考状态展示
10. 更新 DeepSeek 预置模型，并提升 MCP 服务兼容性
11. 多项问题修复与稳定性优化

## v1.21.1 - 2026.06.12
1. 修复上传文件后发送按钮无法恢复可用的问题
2. 更新 Android 应用图标
...
4. Tavily 搜索新增 parse_link 工具，可抓取网页正文获得更丰富的搜索结果
5. 原始错误信息新增「翻译」按钮
6. 升级 Mermaid 版本，支持维恩图（Venn diagrams）
7. 调整输入框字号
8. 修复对话历史中工具调用部分丢失的问题
9. 修复最大上下文消息数为 0 时输入内容丢失
10. 修复清空所有对话后标题栏消失
...
2. 统一模型列表，自动获取各服务商最新模型信息
3. 重新设计思考过程和联网搜索的展示方式，AI 推理过程更直观
4. 全新 Copilot 系统，提供预配置的 AI 助手，方便快速开始常见任务
5. 代码高亮切换为 Shiki，支持更多语言和主题
6. 支持设置会话背景图片，打造个性化聊天界面
7. 新增通义千问、MiniMax、Moonshot、Anthropic 的 OAuth 登录支持
8. 新增消息布局设置引导
9. 优化小屏幕下的侧边栏布局
10. 字体大小设置现在仅影响聊天消息，不再影响整个界面
11. 修复 Token 用量显示不准确的问题，现在显示 AI 服务商返回的实际用量
12. 修复 Android 端图片生成的稳定性问题
13. 多项问题修复和 UI 优化
